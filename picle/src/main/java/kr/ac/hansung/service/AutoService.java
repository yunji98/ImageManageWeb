...
public class AutoService {
	...
	// 해당 tag image 가져오기
		public List<Auto> getTagProducts(String tag) throws InterruptedException, ExecutionException {
			try {
				List<Auto> autoList = new ArrayList<Auto>();
				List<String> tokenList = new ArrayList<String>();
				...
				ApiFuture<QuerySnapshot> future = dbFirestore.collection(COL_NAME).whereEqualTo(tag, true).get();
				List<QueryDocumentSnapshot> documents = future.get().getDocuments();
				int num = 0;			
